...
func getStatus(res http.ResponseWriter, req *http.Request, p httprouter.Params) {
	...
	btn := temp["result"].(map[string]interface{})["availableBalance"].(float64) / divisor
	temp["result"].(map[string]interface{})["availableBalance"] = btn
	btn = temp["result"].(map[string]interface{})["lockedAmount"].(float64)
	temp["result"].(map[string]interface{})["lockedAmount"] = btn / divisor
